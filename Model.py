    ...
    def __init__(self, encoder_q, encoder_k, queue_feature_dim, queue_size=65536, momentum=0.999, temperature=0.07, mlp=False, return_q = False):
        ...
        if mlp: # if projection head is used
            self.projection_head_q = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(self.encoder_q.feature_dim, self.projection_dim)
            )
...
# define the ContrastiveLearningEncoderMeanPooling module without transformer decoder, substitute it with a mean pooling
class ContrastiveLearningEncoderMeanPooling(nn.Module):
    # init a wav2vec2 model
    def __init__(self, wav2vec2_path):
        super(ContrastiveLearningEncoderMeanPooling, self).__init__()
        # self.wav2vec2_model.config.hidden_size = 1024    
        # self.wav2vec2_model = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-large-xlsr-53")  # load the wav2vec2 model from huggingface
        self.wav2vec2_model:Wav2Vec2Model = Wav2Vec2Model.from_pretrained(wav2vec2_path)
    ...
    def forward(self, x):
        ...
        last_hidden_state = self.wav2vec2_model(x).last_hidden_state  # the last_hidden_state has shape (batch_size, 201, 1024), 201 is the number of frames, 1024 is the dimension of the feature vector
        # mean pooling
        return torch.mean(last_hidden_state, dim=1)  # the output should have shape (batch_size, 1024)
...
class RawNetEncoder(nn.Module):
    def __init__(self, d_args, device):
        ...
        # d_args['filts'][2][0] = d_args['filts'][2][1]  # changed by CLAD23 team since this makes this class can't be instantiated twice.
